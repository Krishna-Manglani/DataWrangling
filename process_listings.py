...
# region_parent_name / region_name are structured fields (set by Inside Airbnb itself),
# so they're far more reliable than free-text host-entered fields like 'neighbourhood'.
CHRISTCHURCH_FILTER_COL_CANDIDATES = [
    "region_parent_name",   # e.g. "Christchurch City" — structured, near-100% populated
    "region_name",          # e.g. ward names within Christchurch
    "city",
    "neighbourhood_cleansed",
    "neighbourhood",        # free text typed by hosts — least reliable, used as last resort
]
CHRISTCHURCH_FILTER_VALUE = "Christchurch"
...
def load_and_filter(filepath: Path, verbose: bool = True) -> pd.DataFrame:
    ...
    mask = df[filter_col].astype(str).str.contains(CHRISTCHURCH_FILTER_VALUE, case=False, na=False)
    n_match = mask.sum()

    if verbose:
        print(f"  filter column used: '{filter_col}' | raw rows: {n_raw} | matched: {n_match}")
        if n_match == 0:
            # Show what values WERE present, so a silent 0 doesn't go unnoticed
